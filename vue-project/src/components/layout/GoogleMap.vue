...
const findPlaces = async (keyword) => {
  ...
  const request = {
    textQuery: keyword,
    fields: ['displayName', 'location', 'photos', 'formattedAddress', 'internationalPhoneNumber'],
    language: 'ko-KR',
  };
  ...
  <div>
    ...
    <div id="mapContainer" class="d-flex">
      ...
      >
        <Marker
          ...
          :icon="selectedPlace?.id === place.id ? 'http://maps.google.com/mapfiles/ms/icons/blue-dot.png' : 'http://maps.google.com/mapfiles/ms/icons/red-dot.png'"
          :options="{ position: { lat: place.location.lat(), lng: place.location.lng() }, title: place.displayName }" 
          @click="showPlaceDetails(place)"
        />
      </GoogleMap>
    </div>
    ...
    <div id="placeDetails" v-if="selectedPlace">
      <div v-if="selectedPlace">
        ...
        <p>위치: {{ selectedPlace.location.lat() }}, {{ selectedPlace.location.lng() }}</p>
