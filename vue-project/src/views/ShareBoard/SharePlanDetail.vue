...
import DailyScheduleShareView from "@/components/layout/DailyScheduleShareView.vue";
...
const fetchSchedulesByDate = async (plan_id) => {
  try {
    const response = await planStore.fetchShareVisitPlaceByDate(plan_id);
    console.log("가져온 데이터:", response.dailySchedules);

    dailySchedules.value = { ...response.dailySchedules };

    // dateWithSchedules 데이터 변환
    dateList.value = Object.keys(dailySchedules.value);
    const mappedSchedules = dateList.value.map((date) => ({
      date,
      places: dailySchedules.value[date],
    }));

    dateWithSchedules.value = mappedSchedules; // 변환 데이터 저장
    console.log("dateWithSchedules 데이터:", dateWithSchedules.value);
  } catch (error) {
...
const dateWithSchedules = computed(() => {
  return dateList.value
    ...
    .sort((a, b) => new Date(a.date) - new Date(b.date)); // 날짜 기준으로 정렬
});
